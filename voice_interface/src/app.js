...
app.setHandler({
        ...
        if (!this.$session.$data.loggedIn) {
            ...
            await idDoc
                ...
                .then(id_doc => {
                    if (!id_doc.exists) {
                        this.ask(
                            "Read for this account is not properly installed yet."
                        );
                    } else {
                        ...
                        return collection
                            ...
                            .then(doc => {
                                if (!doc.exists) {
                                    this.ask(
                                        "No data was found for this account"
                                    );
                                } else {
    ...
    DeleteConfirmationState: {
        async YesIntent() {
            ...
            var deletedId = ''
            var targetId = '';
            await db.collection(username).orderBy('date', 'desc').limit(3).get().then(
                documents => {
                    documents.forEach(
                        document => {
                            if (counter == 0) {

...
export class InputModalComponent implements OnInit {
  ...
  // transcript = '';
  isRecording = false;
  constructor(private conm: CommunicationService, private injector: Injector) {
    this.isChrome = window.navigator.userAgent.indexOf("Chrome") !== -1;
    if (this.isChrome) {
      this.speechService = injector.get(SpeechRecognitionService);
  ...
  onListen() {
    ...
    if (this.isChrome) {
      ...
      console.log("Speech Recognition is Only Supported in Chrome");
    }
  }
  ...
  getName(mode) {
    switch (mode) {
      case 0:
        this.placeholder =
          "Please describe what you ate in whatever form would be most useful for you.";
        return "Description";
      case 1:
        this.placeholder =
          "Please describe the terms you would use to search from a database of foods commonly eaten and logged.";
        return "Search";
      case 2:
        return "Image";
      case 3:
        this.placeholder =
          "Please enter the URL of a recipe or other website which describes what you ate.";
        return "URL";
      case 4:
        return "Voice";
      case 5:
  ...
  onSave() {
    if (
      this.isText(this.mode) ||
      this.isVoice(this.mode) ||
      this.isSearch(this.mode)
    ) {
